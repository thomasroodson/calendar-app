<script lang="ts">
  ...
  const scrollToMinutes = (minutes: number) => {
    ...
    const top = Math.max(0, minutes * MINUTE_HEIGHT - 120);
  ...
  $effect(() => {
    ...
    const rawStart = new Date(ev.startDate);
    const rawEnd = new Date(ev.endDate);

    const { start, isVisible } = clampEventToDay(rawStart, rawEnd, date);
    if (!isVisible) return;

    const startMinutes = start.getHours() * 60 + start.getMinutes();
    scrollToMinutes(startMinutes);
  });
</script>
